...
for _, key in ipairs(conflicting_keys) do
  pcall(function() vim.keymap.del({"n", "i", "v"}, key) end)
end
...
vim.api.nvim_create_autocmd("FileType", {
  ...
  callback = function()
    ...
    vim.keymap.set({ "n", "i", "v" }, "<2-LeftMouse>", auto_open_file, { buffer = bufnr, noremap = true, silent = true })
...
pcall(function() vim.keymap.del("i", "<C-/>") end)
pcall(function() vim.keymap.del("v", "<C-/>") end)
pcall(function() vim.keymap.del("n", "<C-_>") end)
pcall(function() vim.keymap.del("i", "<C-_>") end)
pcall(function() vim.keymap.del("v", "<C-_>") end)
...
vim.keymap.set("n", "<F9>", function()
  ...
  if copilot_active then
      ...
      if ok_copilot then copilot.status.data.status = "Disabled" end

      if pcall(require, "codeium") then
          pcall(vim.cmd, "CodeiumEnable")
          vim.g.codeium_enabled = true  -- <-- add this
          vim.notify("Codeium enabled", vim.log.levels.INFO)
      end
  else
      if pcall(require, "codeium") then
          pcall(vim.cmd, "CodeiumDisable")
          vim.g.codeium_enabled = false  -- <-- add this
      end
      vim.cmd("Copilot enable")
      vim.notify("Copilot enabled", vim.log.levels.INFO)
  end

  -- Only update barbecue (no lualine.refresh)
